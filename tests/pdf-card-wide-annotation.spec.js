    ...
    try {
        ...
        for(let attempt = 0; attempt < 100 && !value; attempt++) {
            ...
            const result = await send('Runtime.evaluate', { returnByValue: true, awaitPromise: true, expression: `(async () => {
                ...
                fixtures.directSourceCards = [
                    ['6a68865f-d305-4b33-b073-63a4536689e1', '6a68860f-60e4-412e-8f73-197a7fd790d8'],
                    ['6a69e75e-a2a8-479f-9693-46066d02fdb5', '6a69e428-694f-489e-8a59-5b86f9306595']
                ].map(([cardId, sourceUuid]) => {
                    const card = cards.find(item => item.id === cardId);
                    const raw = document.createElement('div'); raw.innerHTML = card?.q || '';
                    const rawRegion = raw.querySelector('.extra-annotation[data-source-uuid="' + sourceUuid + '"]');
                    const rendered = card && createDOM(card, true);
                    const region = rendered?.querySelector('.mk-pdf-ref-text[data-source-uuid="' + sourceUuid + '"]');
                    const icon = region?.querySelector(':scope > .mk-pdf-annotation-icon');
                    const data = icon && JSON.parse(icon.dataset.annotation || 'null');
                    return { cardId, sourceUuid, rawPreserved: !!rawRegion, icon: !!icon, handler: region?.dataset.mkPdfRegionBound === '1', lookup: pdfAnnotationSourceIndex.get(sourceUuid) || null, deepLink: data && buildPdfHelperDeepLink(data) };
                });

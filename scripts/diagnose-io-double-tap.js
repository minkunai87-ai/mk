...
async function main() {
    ...
    try {
        ...
        const cycleResult = await evaluate(`({scale:document.querySelector('#question-section .mk-io-wrapper')._mediaZoomState.scale,overlayCount:document.querySelectorAll('.mk-high-res-zoom-layer').length,sharpLayerCount:document.querySelectorAll('.mk-io-sharp-viewport-layer').length})`);
        ...
        if(cycleResult.scale !== 1 || cycleResult.overlayCount !== 0 || cycleResult.sharpLayerCount !== 0) throw new Error('IO zoom DOM accumulated after 20 cycles: '+JSON.stringify(cycleResult));

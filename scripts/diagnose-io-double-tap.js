...
async function main() {
    ...
    try {
        ...
        const inlineZoom = await evaluate(`(() => { const wrapper=document.querySelector('#question-section .mk-io-wrapper'); return {scale:wrapper._mediaZoomState.scale,wrapperTransform:wrapper.style.transform,overlayCount:document.querySelectorAll('.mk-high-res-zoom-layer').length,ioImages:wrapper.querySelectorAll('img').length,svgMasks:wrapper.querySelectorAll('svg,.mk-io-layer').length}; })()`);
        ...
        await delay(100);
        const pinchZoom = await evaluate(`(() => {const wrapper=document.querySelector('#question-section .mk-io-wrapper');return {scale:wrapper._mediaZoomState.scale,wrapperTransform:wrapper.style.transform,overlayCount:document.querySelectorAll('.mk-high-res-zoom-layer').length};})()`);
        ...
        await delay(100);
        const maxZoom = await evaluate(`(() => {const wrapper=document.querySelector('#question-section .mk-io-wrapper');return {scale:wrapper._mediaZoomState.scale,transform:wrapper.style.transform};})()`);
        ...
        if(inlineZoom.scale !== 1.7 || !inlineZoom.wrapperTransform.includes('scale(1.7)') || inlineZoom.overlayCount !== 0 || inlineZoom.ioImages !== 1 || inlineZoom.svgMasks !== 1) throw new Error('IO inline zoom restoration verification failed: '+JSON.stringify(inlineZoom));
        if(pinchZoom.scale <= 1.7 || !pinchZoom.wrapperTransform.includes('scale(') || pinchZoom.overlayCount !== 0) throw new Error('IO inline pinch verification failed');

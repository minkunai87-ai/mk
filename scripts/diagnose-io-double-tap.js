...
async function main() {
    ...
    try {
        ...
        await delay(100);
        const maxZoom = await evaluate(`(() => {const wrapper=document.querySelector('#question-section .mk-io-wrapper'),img=wrapper.querySelector('img');return {scale:wrapper._mediaZoomState.scale,rasterFactor:wrapper._mediaZoomState.ioRasterFactor,rasterWidth:img.clientWidth,transform:wrapper.style.transform};})()`);
        ...
        if(inlineZoom.scale !== 1.7 || inlineZoom.rasterWidth <= imageDimensions.client[0] || !inlineZoom.wrapperTransform.includes('scale(1.7)') || inlineZoom.overlayCount !== 0 || inlineZoom.ioImages !== 1 || inlineZoom.svgMasks !== 1 || inlineZoom.maskDelta.some(delta=>delta>0.1)) throw new Error('IO inline zoom restoration verification failed: '+JSON.stringify(inlineZoom));
        if(pinchZoom.scale <= 1.7 || pinchZoom.rasterWidth <= inlineZoom.rasterWidth || !pinchZoom.wrapperTransform.includes('scale(') || pinchZoom.overlayCount !== 0) throw new Error('IO inline pinch verification failed: '+JSON.stringify(pinchZoom));

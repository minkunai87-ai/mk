...
async function main() {
    ...
    try {
        ...
        const inlineZoom = await evaluate(`(() => { const wrapper=document.querySelector('#question-section .mk-io-wrapper'),img=wrapper.querySelector('img'),layer=wrapper.querySelector('.mk-io-layer'),canvas=document.querySelector('.mk-io-crop-canvas'),a=img.getBoundingClientRect(),b=layer.getBoundingClientRect(),c=canvas?.getBoundingClientRect(); return {scale:wrapper._mediaZoomState.scale,rasterFactor:wrapper._mediaZoomState.ioRasterFactor,rasterWidth:img.clientWidth,wrapperWidth:wrapper.clientWidth,wrapperTransform:wrapper.style.transform,imageTransform:img.style.transform,overlayCount:document.querySelectorAll('.mk-high-res-zoom-layer').length,canvasCount:document.querySelectorAll('.mk-io-crop-canvas').length,canvasCss:c?[c.width,c.height]:null,canvasBitmap:canvas?[canvas.width,canvas.height]:null,renderDpr:Number(canvas?.dataset.renderDpr||0),sourceCrop:canvas?.dataset.sourceCrop||'',ioImages:wrapper.querySelectorAll('img').length,svgMasks:wrapper.querySelectorAll('svg,.mk-io-layer').length,maskDelta:[Math.abs(a.left-b.left),Math.abs(a.top-b.top),Math.abs(a.width-b.width),Math.abs(a.height-b.height)]}; })()`);
